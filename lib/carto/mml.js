...
// Add join funcion to join array values into a space-separated string
const JoinSpaceType = new yaml.Type('!join', {
  kind: 'sequence',
...
// Add join function by adding JoinSpaceType to default schema.
// When upgrading to v4 use yaml.DEFAULT_SCHEMA.extend.
const JOIN_SCHEMA = yaml.Schema.create(yaml.DEFAULT_SCHEMA, [ JoinSpaceType ]);
// Usage:
//      foo: !join [ "bar", "baz" ]
//   or
//      foo: !join
...
carto.MML.prototype.load = function load(basedir, data, callback) {
    ...
    try {
        mml = yaml.safeLoad(data, { schema: JOIN_SCHEMA });
    } catch (err) {
        env = {};
        util.error(env, {
